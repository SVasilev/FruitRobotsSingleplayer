...
Minimax.prototype.getDirection = function() {
  ...
  this.graph[startState].forEach(function(neighbour) {
    var neighbourResult = this._minimaxAlgorithm(neighbour, Number.NEGATIVE_INFINITY, Number.POSITIVE_INFINITY, 'player');
...
Minimax.prototype._minimaxAlgorithm = function(state, alpha, beta, player) {
  ...
  if (player === 'ai') {
    ...
    for (var i = 0; i < neighbours.length; i++) {
      maxResult = Math.max(maxResult, this._minimaxAlgorithm(neighbours[i], alpha, beta, 'player'));
    ...
    for (var i = 0; i < neighbours.length; i++) {
      minResult = Math.min(minResult, this._minimaxAlgorithm(neighbours[i], alpha, beta, 'ai'));
